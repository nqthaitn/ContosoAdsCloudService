...
namespace ContosoAdsWeb.Controllers
{
    public class AdController : Controller
    {
        ...
        // GET: Ad
        public async Task<ActionResult> Index(int? categogy)
        {
            ...
            if (categogy != null)
            {
                adsList = adsList.Where(a => a.Category == (Category)categogy);
            }
        ...
        public async Task<ActionResult> Edit(
            ...
            HttpPostedFileBase imageFile)
        {
            CloudBlockBlob imageBolb = null;
            if (ModelState.IsValid)
            {
                if (imageFile != null && imageFile.ContentLength != 0)
                {
                    await DeleteAdBlobsAsync(ad);
                    imageBolb = await UploadAndSaveBlobAsync(imageFile);
                    ad.ImageURL = imageBolb.Uri.ToString();
                }
        ...
        private async Task DeleteAdBlobsAsync(Ad ad)
        {
            if (!string.IsNullOrWhiteSpace(ad.ImageURL))
            {
                ...
                await DeleteAdBlobsAsync(blobUri);
            }
        ...
        private async Task DeleteAdBlobsAsync(Uri blobUri)
        {
